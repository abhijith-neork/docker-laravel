...
{ 
    ...
    {
        $count = Policy::select('count(*) as allcount')->where(function ($query) use ($value) {
            $query->where('policy_title', 'like', '%' . $value . '%')
            ->orWhere('policy_description', 'like', '%' . $value . '%')
            ->orWhere('effective_from', 'like', '%' . $value . '%')
    ...
    public function get_records($columnName, $columnSortOrder, $searchValue, $start, $rowperpage, $status = '')
    {
        $records = Policy::orderBy($columnName, $columnSortOrder)
            ->where(function ($query) use ($searchValue) {
                $query->where('policy_title', 'like', '%' . $searchValue . '%')
                ->orWhere('policy_description', 'like', '%' . $searchValue . '%')
                ->orWhere('effective_from', 'like', '%' . $searchValue . '%')
        ...
        if ($status != '') {
            $records->where('policies.status', $status);
        }
